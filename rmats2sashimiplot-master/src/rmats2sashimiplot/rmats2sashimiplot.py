...
def plot_e(options, id_str, gene_symbol, events_no):
    ...
    if options.group_info is not None:
        os.system("python {0} --plot-event \"{1}\" {2} {3} "
                  "--output-dir {4} --group-info {5}".format(path_sashimi_plot, id_str.lstrip('chr'), out_index, setting_str,
                                                             output_path, options.group_info))
    else:
        os.system("python {0} --plot-event \"{1}\" {2} {3} "
                  "--output-dir {4}".format(path_sashimi_plot, id_str.lstrip('chr'), out_index, setting_str, output_path))
...
def plot_with_coordinate(options):
    ...
    try:
        ...
        try:
            ...
            sys.exit(-2)
        print 'start call function plot_c  the id str is: {}'.format(id_str)
