...
def vec2psi(inc, skp, effective_inclusion_length, effective_skipping_length):
    ...
    for i in range(len(inc)):
        if (float(inc[i]) + float(skp[i])) == 0:
            ...
            psi.append(str(round(
                float(inc[i]) / inclusion_length / (float(inc[i]) / inclusion_length + float(skp[i]) / skipping_length),
                3)));
    return (psi);
...
for i in ilines:
    element = re.findall('[^\t\n]+', i);
    inc1 = re.findall('[^,]+', element[0]);
    skp1 = re.findall('[^,]+', element[1]);
    inc2 = re.findall('[^,]+', element[2]);
    skp2 = re.findall('[^,]+', element[3]);
    inc1 = vec2float(inc1);
    skp1 = vec2float(skp1);
    inc2 = vec2float(inc2);
    skp2 = vec2float(skp2);
