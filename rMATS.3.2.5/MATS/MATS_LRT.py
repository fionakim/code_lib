...
def myfunc_multivar(x, *args):
    ...
    sum1 = sum1 / var2 / 2;
    return (sum1 + 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
...
def myfunc_multivar_der(x, *args):
    ...
    sum2 = 0;
    for i in range(len(psi1)):
        sum1 += -2 * (logit((psi1[i] - psi2[i]) / 2 + 0.5) - logit((x[0] - x[1]) / 2 + 0.5)) / (
            (x[0] - x[1]) / 2 + 0.5) / (0.5 - (x[0] - x[1]) / 2) * 0.5;
    sum1 = sum1 / var1 / 2;
    ...
    for i in range(len(psi2)):
        sum2 += -2 * (logit((psi1[i] - psi2[i]) / 2 + 0.5) - logit((x[0] - x[1]) / 2 + 0.5)) / (
            (x[0] - x[1]) / 2 + 0.5) / (0.5 - (x[0] - x[1]) / 2) * (-0.5);
    sum2 = sum2 / var2 / 2;
...
def myfunc_1(x, *args):
    ...
    beta2 = args[2][1];
    var = args[3];
    effective_inclusion_length = args[4];
    effective_skipping_length = args[5];
    inclusion_length = effective_inclusion_length;
    skipping_length = effective_skipping_length;
    new_psi1 = inclusion_length * (x + cutoff) / (
        inclusion_length * (x + cutoff) + skipping_length * (1 - (x + cutoff)));
    new_psi2 = inclusion_length * x / (inclusion_length * x + skipping_length * (1 - x));
    binomial_sum = -1 * (I1 * log(new_psi1) + S1 * log(1 - new_psi1) + I2 * log(new_psi2) + S2 * log(1 - new_psi2));
    multivar_sum = 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
...
def myfunc_der_1(x, *args):
    ...
    new_psi1 = inclusion_length * (x + cutoff) / (
        inclusion_length * (x + cutoff) + skipping_length * (1 - (x + cutoff)));
    new_psi2 = inclusion_length * x / (inclusion_length * x + skipping_length * (1 - x));
    new_psi1_der = inclusion_length * skipping_length / pow(
        inclusion_length * (x + cutoff) + skipping_length * (1 - (x + cutoff)), 2);
    ...
    res1 += 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
    ...
    res2 += 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
...
def myfunc_2(x, *args):
    ...
    inclusion_length = effective_inclusion_length;
    skipping_length = effective_skipping_length;
    new_psi1 = inclusion_length * (x) / (inclusion_length * (x) + skipping_length * (1 - (x)));
    new_psi2 = inclusion_length * (x + cutoff) / (
        inclusion_length * (x + cutoff) + skipping_length * (1 - (x + cutoff)));
    binomial_sum = -1 * (I1 * log(new_psi1) + S1 * log(1 - new_psi1) + I2 * log(new_psi2) + S2 * log(1 - new_psi2));
    multivar_sum = 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
        pow(stats.norm.ppf(x), 2) + pow(stats.norm.ppf(x + cutoff), 2) - 2 * rho * stats.norm.ppf(x) * stats.norm.ppf(
            x + cutoff))
    return (binomial_sum + multivar_sum);
...
def myfunc_der_2(x, *args):
    ...
    new_psi2_der = inclusion_length * skipping_length / pow(
        inclusion_length * (x + cutoff) + skipping_length * (1 - (x + cutoff)), 2);
    res1 = -1 * (I1 / new_psi1 - S1 / (1 - new_psi1)) * new_psi1_der;
    res1 += 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
        2 * stats.norm.ppf(x) - 2 * rho * stats.norm.ppf((x + cutoff))) / stats.norm.pdf(stats.norm.ppf(x))
...
def myfunc_individual(x, *args):
    ...
    inclusion_length = effective_inclusion_length;
    skipping_length = effective_skipping_length;
    new_psi1 = inclusion_length * x[0] / (inclusion_length * x[0] + skipping_length * (1 - x[0]));
    new_psi2 = inclusion_length * x[1] / (inclusion_length * x[1] + skipping_length * (1 - x[1]));
    binomial_sum = -1 * (I1 * log(new_psi1) + S1 * log(1 - new_psi1) + I2 * log(new_psi2) + S2 * log(1 - new_psi2));
    multivar_sum = 0.1 * 0.5 * (pow(rho, 2)) / (1 - pow(rho, 2)) * (
        pow(stats.norm.ppf(x[0]), 2) + pow(stats.norm.ppf(x[1]), 2) - 2 * rho * stats.norm.ppf(x[0]) * stats.norm.ppf(
            x[1]))
...
def myfunc_likelihood(x, args):
    ...
    if (N1 + N2) == 0:
        return (0);
    sum += -0.5 * (
        (I1 - N1 * x[0]) * (I1 - N1 * x[0]) / (N1 * x[0]) + (S1 - N1 * (1 - x[0])) * (S1 - N1 * (1 - x[0])) / (
            N1 * (1 - x[0])));
    sum += -0.5 * (
        (I2 - N2 * x[1]) * (I2 - N2 * x[1]) / (N2 * x[1]) + (S2 - N2 * (1 - x[1])) * (S2 - N2 * (1 - x[1])) / (
            N2 * (1 - x[1])));
...
def MLE_iteration_constrain(i1, i2, s1, s2, effective_inclusion_length, effective_skipping_length):
    ...
    while ((iter_cutoff > 0.01) & (count <= iter_maxrun)):
        ...
        if (sum(psi1) / len(psi1)) > (sum(psi2) / len(psi2)):  # minize psi2 if this is the case
            xopt = fmin_l_bfgs_b(myfunc_1, [sum(psi2) / len(psi2)], myfunc_der_1,
                                 args=[[i1[0], i2[0]], [s1[0], s2[0]], [beta_0, beta_1], var1,
                                       effective_inclusion_length, effective_skipping_length],
            ...
            xopt = fmin_l_bfgs_b(myfunc_2, [sum(psi1) / len(psi1)], myfunc_der_2,
                                 args=[[i1[0], i2[0]], [s1[0], s2[0]], [beta_0, beta_1], var1,
                                       effective_inclusion_length, effective_skipping_length],
                                 bounds=[[0.001, 0.999 - cutoff]], iprint=-1)
        ...
        new_psi1.append(theta1);
        new_psi2.append(theta2);
        psi1 = new_psi1;
        psi2 = new_psi2;
        if count > 1:
...
def MLE_iteration(i1, i2, s1, s2, effective_inclusion_length, effective_skipping_length):
    ...
    while ((iter_cutoff > 0.01) & (count <= iter_maxrun)):
        ...
        for i in range(len(psi1)):
            xopt = fmin_l_bfgs_b(myfunc_individual, [psi1[i], psi2[i]], myfunc_individual_der,
                                 args=[[i1[i], i2[i]], [s1[i], s2[i]], [beta_0, beta_1], var1,
                                       effective_inclusion_length, effective_skipping_length],
            ...
            # Debug;print(xopt);
            likelihood_sum += myfunc_likelihood([new_psi1[i], new_psi2[i]],
                                                [[i1[i], i2[i]], [s1[i], s2[i]], [beta_0, beta_1], var1]);
        psi1 = new_psi1;
        psi2 = new_psi2;
...
def likelihood_test(i1, i2, s1, s2, effective_inclusion_length, effective_skipping_length, flag):
    if flag == 0:
        ...
        if abs(res[1][2] - res[1][3]) <= cutoff:
            ...
            res_constrain = MLE_iteration_constrain(i1, i2, s1, s2, effective_inclusion_length,
...
for i in range(len(psi1_for_rho_list)):
    this_rho = stats.pearsonr(numpy.array(psi1_for_rho_list[i]), numpy.array(psi2_for_rho_list[i]));
    this_rho = this_rho[0];
    if this_rho > 0.9:
        this_rho = 0.9;
