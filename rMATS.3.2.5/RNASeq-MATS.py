...
validArgList = ['-s1', '-b1', '-s2', '-b2', '-gtf', '-bi', '-o', '-t', '-len', '-a', '-r1', '-r2', '-sd1', '-sd2', '-c',
                '-analysis', '-keepTemp', '-libType', '-lite', '-novelSS'];
...
if lite == 0:
    if (
                                        s1 == '' or s2 == '' or gtf == '' or bIndex == '' or outDir == '' or readLength == 0 or readType == ''):  ### at least one required param is missing
    ...
    if ((
                        s1 == '' and s2 == '') or gtf == '' or bIndex == '' or outDir == '' or readLength == 0 or readType == ''):  ### at least one required param is missing
        print ('Not enough arguments!!');
        print (
            'Usage (with fastq files):\n\tpython RNASeq-MATS.py -s1 rep1_1[:rep1_2][,rep2_1[:rep2_2]]* -s2 rep1_1[:rep1_2][,rep2_1[:rep2_2]]* -gtf gtfFile -bi bowtieIndexBase -o outDir -t readType -len readLength [options]*');
        print (
...
if lite == 0:
    if bamFile == 1 and (((sample_1[0].split('.'))[-1].strip()).upper() != 'BAM' or (
            (sample_2[0].split('.'))[-1].strip()).upper() != 'BAM'):
    ...
    for fq in sample_1:  ## examine each file
        ...
        for yyy in fpfp:  ## for each fastq file
            ...
            if len(line2) != readLength:  ### different readLength
                print "Incorrect readLength. %s has a read length of %d, while readLength param is %d" % (
                    yyy, len(line2), readLength);
                fff.close();
                sys.exit();
            fff.close();
    if lite == 0:
...
def doSTARMapping():  ## do STAR mapping
    ...
    for rr in range(0, len(sample_1)):  ## for each replicate of sample_1
        ...
        cmd += '--alignSJDBoverhangMin ' + str(
            tophatAnchor) + ' --alignIntronMax 300000 --genomeDir ' + bIndex + ' --sjdbGTFfile ' + gtf;
...
def doTophatMapping():  ## do tophat mapping
    ...
    for rr in range(0, len(sample_1)):  ## for each replicate of sample_1
        rTempFolder = s1rPath + str(rr + 1);
        cmd = 'tophat -a ' + str(
            tophatAnchor) + ' -m 0 -I 300000 -p 4 -g 20 --library-type ' + libType + ' --no-novel-indels ';
        cmd += ' -N 3 --segment-mismatches 2 -G ' + gtf + ' -o ' + rTempFolder;
        if SEPE == 'PE':  ## paired-end
            cmd += ' -r ' + str(insertLength[rr]) + ' --mate-std-dev ' + str(sigma[rr]) + ' ' + bIndex + ' ' + \
                   sample_1[rr].split(':')[0] + ' ' + sample_1[rr].split(':')[1];
        else:  ## single-end
    ...
    for rr in range(0, len(sample_2)):  ## for each replicate of sample_2
        ...
        cmd += ' -N 3 --segment-mismatches 2 -G ' + gtf + ' -o ' + rTempFolder;
        if SEPE == 'PE':  ## paired-end
            cmd += ' -r ' + str(insertLength2[rr]) + ' --mate-std-dev ' + str(sigma2[rr]) + ' ' + bIndex + ' ' + \
                   sample_2[rr].split(':')[0] + ' ' + sample_2[rr].split(':')[1];
        else:  ## single-end
...
def getUniqueSAM():  ## getting uniquely mapped reads or pairs, DEPRECATED!!
    ...
    for rr in range(0, len(sample_1)):  ## for each replicate of sample_1
        ...
        if SEPE == 'PE':  ## paired-end
            ...
            cmd += ' | awk -F"\\t" \'(($0 ~ "NH:i:1[^0-9]"||$0 ~ "NH:i:1$") && ((and($2,0x2))&&($6=="' + str(
                readLength) + 'M"||($6~"N"&&$6!~"D"&&$6!~"I"))) )|| NF<7\' > ' + rTempFolder + '/unique.S1.sam';  ## genome, junction reads and header
        else:  ## single-end
            if bamFile == 0:
                cmd += 'samtools view -h ' + rTempFolder + '/Aligned.sortedByCoord.out.bam';
            else:
                cmd += 'samtools view -h ' + sample_1[rr];
            cmd += ' | awk -F"\\t" \'(($0 ~ "NH:i:1[^0-9]"||$0 ~ "NH:i:1$") && ($6=="' + str(
                readLength) + 'M"||($6~"N"&&$6!~"D"&&$6!~"I")) )|| NF<7\' > ' + rTempFolder + '/unique.S1.sam';  ## genome, junction reads and header
        oFile.write(
            '######  getting unique reads or pairs for sample_1, replicate_' + str(rr + 1) + '#####\n' + cmd + '\n#\n');
        oFile.flush();
        status, output = commands.getstatusoutput(cmd);
        logging.debug(
            "getting uniquely mapped reads or pairs for sample_1, rep_%d is done with status %s" % ((rr + 1), status));
        if (int(status) != 0):  ## it did not go well
            ...
            for rp in range(0, 3):  ## try up to 3 more times
                ...
                status, output = commands.getstatusoutput(cmd);
                logging.debug("getting uniquely mapped reads or pairs for sample_1, rep_%d is done with status %s" % (
                    (rr + 1), status));
                if (int(status) == 0):  ## worked okay
                    break;  ## break for loop
                else:  ### error
                    logging.debug(
                        "error in getting uniquely mapped reads from smaple_1, rep_%d: %s" % ((rr + 1), status));
                    logging.debug("error detail: %s" % output);
    ...
    for rr in range(0, len(sample_2)):  ## for each replicate of sample_2
        ...
        if SEPE == 'PE':  ## paired-end
            ...
            cmd += ' | awk -F"\\t" \'(($0 ~ "NH:i:1[^0-9]"||$0 ~ "NH:i:1$") && ((and($2,0x2))&&($6=="' + str(
                readLength) + 'M"||($6~"N"&&$6!~"D"&&$6!~"I"))) )|| NF<7\' > ' + rTempFolder + '/unique.S2.sam';  ## genome, junction reads and header
        else:  ## single-end
            if bamFile == 0:
                cmd += 'samtools view -h ' + rTempFolder + '/Aligned.sortedByCoord.out.bam';
            else:
            ...
            cmd += ' | awk -F"\\t" \'(($0 ~ "NH:i:1[^0-9]"||$0 ~ "NH:i:1$") && ($6=="' + str(
                readLength) + 'M"||($6~"N"&&$6!~"D"&&$6!~"I")) )|| NF<7\' > ' + rTempFolder + '/unique.S2.sam';  ## genome, junction reads and header
        oFile.write(
            '######  getting unique reads or pairs for sample_2, replicate_' + str(rr + 1) + '#####\n' + cmd + '\n#\n');
        oFile.flush();
        status, output = commands.getstatusoutput(cmd);
        logging.debug(
            "getting uniquely mapped reads or pairs for sample_2, rep_%d is done with status %s" % ((rr + 1), status));
        if (int(status) != 0):  ## it did not go well
            ...
            for rp in range(0, 3):  ## try up to 3 more times
                logging.debug("Retry: " + str(rp + 1));
                status, output = commands.getstatusoutput(cmd);
                logging.debug("getting uniquely mapped reads or pairs for sample_2, rep_%d is done with status %s" % (
                    (rr + 1), status));
                if (int(status) == 0):  ## worked okay
                    break;  ## break for loop
                else:  ### error
                    logging.debug(
                        "error in getting uniquely mapped reads from smaple_2, rep_%d: %s" % ((rr + 1), status));
                    logging.debug("error detail: %s" % output);
            if (int(status) != 0):  ## didn't go well in all retries
                raise Exception();
        logging.debug(output);
...
def getASEvents(asDir, ):  ## get AS events from GTF and SAM files
    ...
    cmd = 'python ' + binPath + '/processGTF.BAMs.py ' + gtf + ' ' + asDir + '/fromGTF' + ' ' + finalSamNames + ' ' + libType + ' ' + SEPE + ' ' + str(
        novelSS) + ' ' + tempPath;
...
def runningMATS(asType):  ## running MATS here
    ...
    cmd += ";" + scriptPath + "/MATS/rMATS.sh -d " + bothIso + " -o " + tempPath + "/" + asType + "out_JC/ -c " + str(
        c) + " -p 4 -t " + analysis;
    ...
    cmd = "awk '{ split($2,ic_1,\",\"); sum_ic_1=0; for (x in ic_1) sum_ic_1 += ic_1[x]; split($4,ic_2,\",\"); sum_ic_2=0; for (x in ic_2) sum_ic_2 += ic_2[x]; split($3,sc_1,\",\"); sum_sc_1=0; for (x in sc_1) sum_sc_1 += sc_1[x]; split($5,sc_2,\",\"); sum_sc_2=0; for (x in sc_2) sum_sc_2 += sc_2[x]; if ( NR==1 || ( (sum_ic_2 + sum_sc_2 > 0) && (sum_ic_1 + sum_sc_1 > 0) && (sum_ic_1 != 0 || sum_ic_2 != 0) && (sum_sc_1 != 0 || sum_sc_2 != 0) && $6!=0 && $7!=0 ) ) {print $0}}' " + allInput + " > " + bothIso;
    cmd += ";" + scriptPath + "/MATS/rMATS.sh -d " + bothIso + " -o " + tempPath + "/" + asType + "out_JCEC/ -c " + str(
        c) + " -p 8 -t " + analysis;
    oFile.write(
        '###### running MATS input for ' + asType + ' using Junction Counts and Reads on target Exon Counts #####\n' + cmd + '\n#\n');
    oFile.flush();
    status, output = commands.getstatusoutput(cmd);
...
def joiningMATS(asType):  ## joining MATS output here
    ...
    cmd += " ; awk '{print $(NF-3),$0}' " + tMATS + " | sort -g | cut -d' ' -f2- > " + finalMATS;
    oFile.write(
        '###### joining MATS results for ' + asType + ' using Junction Counts and Reads on target Exon Counts #####\n' + cmd + '\n#\n');
    oFile.flush();
    status, output = commands.getstatusoutput(cmd);
...
def liteJoiningMATS(asType):  ## joining MATS lite here (for future release)
    ...
    oFile.write(
        '###### joining MATS lite results for ' + asType + ' using Junction Counts only #####\n' + cmd + '\n#\n');
    oFile.flush();
    status, output = commands.getstatusoutput(cmd);
    ...
    cmd += ";paste " + allEvents + " " + tMATS + " > " + finalMATS;
    oFile.write(
        '###### joining MATS lite results for ' + asType + ' using Junction Counts and Reads on target Exon Counts #####\n' + cmd + '\n#\n');
    oFile.flush();
    status, output = commands.getstatusoutput(cmd);
...
def printStats(asType, sF):  ## printing stats here
    ...
    logging.debug(
        "getting stats for %s using JC is done with status %s,%s and %s" % (asType, status0, status1, status2));
    ...
    print('Junction Counts Only: There are %d AS events. Of these, %d events are statistically significant' % (
        int(out0), int(out1) + int(out2)));
    print ('%d significant events have higher inclusion level for SAMPLE_1 and %d events for SAMPLE_2' % (
        int(out1), int(out2)));
    ...
    logging.debug('Junction Counts Only: There are %d AS events. Of these, %d events are statistically significant' % (
        int(out0), int(out1) + int(out2)));
    logging.debug('%d significant events have higher inclusion level for SAMPLE_1 and %d events for SAMPLE_2' % (
        int(out1), int(out2)));
    ...
    status2, output2 = commands.getstatusoutput(cmd_2);
    logging.debug(
        "getting stats for %s using JCEC is done with status %s,%s and %s" % (asType, status0, status1, status2));
    if (int(status0) != 0):  ## it did not go well
    ...
    print(
        'Junction Counts and Reads on target Exon Counts: There are %d AS events. Of these, %d events are statistically significant' % (
            int(out0), int(out1) + int(out2)));
    print ('%d significant events have higher inclusion level for SAMPLE_1 and %d events for SAMPLE_2' % (
        int(out1), int(out2)));
