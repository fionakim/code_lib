...
import re, os, sys, logging, time, datetime, pickle, pysam;
...
if (len(sys.argv) < 6):
    ...
    print ('Usage:\n\tpython ProgramName.py gtfFile outputPrefix BAMfiles libType SEPE novelSS [logFolder]');
    print (
        'Example\n\tpython ProgramName.py AceView.ncbi_37.gtf fromAceView sample1.bam,sample2.bam,... fr-unstranded PE 1 temp');
    sys.exit();
...
logFolder = '';
if len(sys.argv) > 7:  ## we have a log folder
    logFolder = sys.argv[7].strip() + '/';
...
SEPE = sys.argv[5];  ## paired or single (PE or SE)
novelSS = int(sys.argv[6]);  ## find novel SS or not
### write out header
...
if (nTx - oneExon) > 0:  ## to avoid divided by zero exception
    logging.debug("Average number of exons per transcript excluding one-exon tx is %f" % (
        float(nExon - oneExon) / (nTx - oneExon)));
#
...
#
#
#### now process BAM files to populate junction dictionaries ####
#
#
def is_unique(r):
...
def getJunctionType(jjj, cgg):
    ...
    jS = int(jEle[-2]);
    jE = int(jEle[-1]);  ## in case chromosome contains '_' in it
    ...
    for txx in genes[cgg]:  ## for each transcript, look for the smallest type, when found type 0, just return it
        ...
        uInd = -1;
        dInd = -1;  ## upstream exon index and downstream exon index
...
def makeNovelTX(jtt, cgg):  ## make novel tx using special junction types
    ...
    E_LA = 2;
    E_RA = 3;
    E_B = 4;  ## novel junction (1), novel exon from left anchor(2), right anchor(3), from both (4)
    rValue[J] = {};
    rValue[E_LA] = {};
    rValue[E_RA] = {};
    rValue[E_B] = {};
    ...
    if 1 in jtt:  ### there are novel junctions
        for jcoord in jtt[1]:
            ...
            jele = jcoord.strip().split('_');
            jS = int(jele[-2]);
            jE = int(jele[-1]);
            
            for txx in genes[
                cgg]:  ## for each transcript, look for the exons with at least one exons between the junction
                cexons = genes[cgg][txx];  ## candidate exons
                uInd = -1;
                dInd = -1;  ## upstream exon index and downstream exon index
                ...
                if uInd > -1 and dInd > -1:  ## we have both exons here.., type 0 or type 1
                    if dInd - uInd > 1:  ## novel junction..
                        key = str(cexons[uInd][0]) + ':' + str(cexons[uInd][1]) + ':' + str(
                            cexons[dInd][0]) + ':' + str(cexons[dInd][1]);  ## novel junction
                        nex = [cexons[uInd], cexons[dInd]];  ## novel exons
                        if uInd > 0:  ## not the first exon
                            key = str(cexons[uInd - 1][0]) + ':' + str(
                                cexons[uInd - 1][1]) + ':' + key;  ## prev exon + novel junction
                            nex = [cexons[uInd - 1]] + nex;
                        if dInd < len(cexons) - 1:  ## not the last exon
                            key = key + ':' + str(cexons[dInd + 1][0]) + ':' + str(
                                cexons[dInd + 1][1]);  ## novel junction + next exon
                            nex = nex + [cexons[dInd + 1]];
                        rValue[J][key] = nex;  ## it's okay to overwrite
                    else:  ### should not be here..
                        logging.debug("Check this out..gene: %s, tx: %s, uExon: %s, dExon: %s" % (
                            cg, txx, cexons[uInd], cexons[dInd]));
    ...
    if novelSS == 0:  ## do not use novel SS
    ...
    if 2 in jtt:  ## novel splice site with left anchor, right side is unknown (could be E_LA or E_B)
        ...
        for jcoord in jtt[2]:
            jele = jcoord.strip().split('_');
            jS = int(jele[-2]);
            jE = int(jele[-1]);  ## jS is known, jE is unknown
                ...
                for ci in range(0, len(cexons)):  ## examine each candidate exon
                    ...
                    if eLength > 0 and eLength < mel:  ## it meets max exon size limit, add a new transcript
                        ...
                        if ci < len(cexons) - 1:  ## not the last exon
                            key = key + ':' + str(cexons[ci + 1][0]) + ':' + str(
                                cexons[ci + 1][1]);  ## novel exon with left anchor
                            nex = nex + [cexons[ci + 1]];
            ...
            if 3 not in jtt:  ### do not do this part. just continue
                continue;
            for rj in jtt[3]:
                rjel = rj.strip().split('_');
                rjS = int(rjel[-2]);
                rjE = int(rjel[-1]);  ## rjS is unknown, rjE is known
                ...
                if eLength > 0 and eLength < mel:  ## it meets max exon size limit, add a new transcript
                    ...
                    key = str(exonStart) + ':' + str(jS) + ':' + str(jE + 1) + ':' + str(rjS) + ':' + str(
                        rjE + 1) + ':' + str(exonEnd);
    ...
    if 3 in jtt:  ## novel splice site with right anchor, left side is unknown (could be E_RA or E_B)
        ...
        for jcoord in jtt[3]:
            jele = jcoord.strip().split('_');
            jS = int(jele[-2]);
            jE = int(jele[-1]);  ## jS is unknown, jE is known
            if jE - jS < mil:  ## intron size does not meet the minimum intron length
                continue;  ## do nothing
            exonEnd = -1;  ## need to get exon end coord
                ...
                for ci in range(0, len(cexons)):  ## examine each candidate exon
                    ...
                    if eLength > 0 and eLength < mel:  ## it meets max exon size limit, add a new transcript
                        ...
                        if ci > 0:  ## not the first exon
                            key = str(cexons[ci - 1][0]) + ':' + str(
                                cexons[ci - 1][1]) + ':' + key;  ## novel exon with right anchor
                            nex = [cexons[ci - 1]] + nex;
            ...
            if 2 not in jtt:  ### skip this
                continue;
            for lj in jtt[2]:
                ljel = lj.strip().split('_');
                ljS = int(ljel[-2]);
                ljE = int(ljel[-1]);  ## ljS is known, ljE is unknown
                ...
                if eLength > 0 and eLength < mel:  ## it meets max exon size limit, add a new transcript
                    ...
                    key = str(exonStart) + ':' + str(ljS) + ':' + str(ljE + 1) + ':' + str(jS) + ':' + str(
                        jE + 1) + ':' + str(exonEnd);
...
#
#
def fullyContainedInInternalExon(myExon,
                                 myGeneID):  ## return true if any transcript has an internal exon fully containing the exon
        ...
        for ce in de:  ## for each exon in distinct exon dictionary
            ...
            for i in range(0, len(uf)):  ### going through the upstream flanking exons
                ...
                for j in range(0, len(
                        df)):  ## going through the downstream falnking exons to see if there is skipping junction
                    f2 = df[j];
                    for tID in genes[gID]:  ## examine each transcript, to see if it has an edge connecting f1 and f2
                        ...
                        for i in range(0, len(genes[gID][tID]) - 1):  ## for each exon in the tx
                            ...
                            if e_1[1] == f1[1] and e_2[0] == f2[
                                0]:  ## this tx has an edge connecting f1 and f2 but does not have ce
                                key = supInfo[1] + ':' + str(ce[0] - 1) + ':' + str(ce[1]) + ':' + str(
                                    f1[1]) + ':' + str(f2[0] - 1);  ## target exon and skipping junction
                                if key in sEvents:  ## already have this skipping events
                                    ...
                                    numSkippingEvents += 1;
                                    oFile_ce.write(
                                        str(numSkippingEvents) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                            ce[0] - 1) + '\t' + str(ce[1]) + '\t' + str(f1[0] - 1) + '\t' + str(
                                            f1[1]) + '\t' + str(f2[0] - 1) + '\t' + str(f2[1]) + '\n');
                                    if (tID.find("novel") >= 0):  # It involves novel junction
                                        neFile_ce.write(str(numSkippingEvents) + '\t' + gID + '\t' + '\t'.join(
                                            supInfo) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\t' + str(
                                            f1[0] - 1) + '\t' + str(f1[1]) + '\t' + str(f2[0] - 1) + '\t' + str(
                                            f2[1]) + '\n');
                                        ###################
            ...
            for i in range(0, len(uf)):  ### going through the upstream flanking exons
                ...
                for j in range(0, len(
                    ...
                    for tID in genes[gID]:  ## examine each transcript
                        if [ce[0], ce[1]] in genes[gID][tID] or f1 not in genes[gID][tID] or f2 not in genes[gID][
                            tID]:  ## ce in, f1 or f2 is not in..do not examine
                            ...
                            if (fromF1 + 1 == fromF2 - 1) and (mxe[0] > ce[1]):  ### this exon is the right one
                                ...
                                if goodMXE:  ### it's okay to write out
                                    ...
                                    key = supInfo[1] + ':' + str(ce[0] - 1) + ':' + str(ce[1]) + ':' + str(
                                        mxe[0] - 1) + ':' + str(mxe[1]) + ':' + str(f1[1]) + ':' + str(f2[0] - 1);
                                    if key in mxEvents:  ## duplicate,
                                        ...
                                        mxEvents[key] = 1;
                                        oFile_mxe.write(
                                            str(numMXEvents) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                                ce[0] - 1) + '\t' + str(ce[1]) + '\t' + str(mxe[0] - 1) + '\t' + str(
                                                mxe[1]) + '\t' + str(f1[0] - 1) + '\t' + str(f1[1]) + '\t' + str(
                                                f2[0] - 1) + '\t' + str(f2[1]) + '\n');
                                        if (tID.find("novel") >= 0):  # It involves novel junction
                                            neFile_mxe.write(
                                                str(numMXEvents) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                                    ce[0] - 1) + '\t' + str(ce[1]) + '\t' + str(
                                                    mxe[0] - 1) + '\t' + str(mxe[1]) + '\t' + str(
                                                    f1[0] - 1) + '\t' + str(f1[1]) + '\t' + str(f2[0] - 1) + '\t' + str(
                                                    f2[1]) + '\n');
            ...
            for i in range(0, len(uf) - 1):  ### going through the upstream flanking exons
                ...
                for j in range(i + 1, len(uf)):
                    ...
                    if e[0] == u[0]:  ## it is alt SS event, because uf is derived from SET
                        ...
                        key = supInfo[1] + ':' + str(min(e[1], u[1])) + ':' + str(max(e[1], u[1])) + ':' + str(
                            ce[0] - 1);
                        if supInfo[2] == '+':  ## positive strand. alt-5 event
                            if key in ss5Events:  ## duplicate
                                ...
                                ss5Events[key] = 1;
                                num5 += 1;
                                oFile_5.write(str(num5) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                    e[0] - 1) + '\t' + str(max(e[1], u[1])) + '\t' + str(e[0] - 1) + '\t' + str(
                                    min(e[1], u[1])) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\n');
                                if (tID.find("novel") >= 0):  # It involves novel junction
                                    neFile_5.write(str(num5) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                        e[0] - 1) + '\t' + str(max(e[1], u[1])) + '\t' + str(e[0] - 1) + '\t' + str(
                                        min(e[1], u[1])) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\n');
                            ...
                            if key in ss3Events:  ## duplicate
                                ...
                                ss3Events[key] = 1;
                                num3 += 1;
                                oFile_3.write(str(num3) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                    e[0] - 1) + '\t' + str(max(e[1], u[1])) + '\t' + str(e[0] - 1) + '\t' + str(
                                    min(e[1], u[1])) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\n');
                                if (tID.find("novel") >= 0):  # It involves novel junction
                                    neFile_3.write(str(num3) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                        e[0] - 1) + '\t' + str(max(e[1], u[1])) + '\t' + str(e[0] - 1) + '\t' + str(
                                        min(e[1], u[1])) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\n');
            ...
            for i in range(0, len(df) - 1):  ### going through the downstream flanking exons
                ...
                for j in range(i + 1, len(df)):
                    ...
                    if e[1] == d[1]:  ## it is alt SS event, because uf is derived from SET
                        key = supInfo[1] + ':' + str(ce[0] - 1) + ':' + str(ce[1]) + ':' + str(
                            min(e[0], d[0]) - 1) + ':' + str(e[1]) + ':' + str(max(e[0], d[0]) - 1) + ':' + str(e[1]);
                        key = supInfo[1] + ':' + str(ce[1]) + ':' + str(min(e[0], d[0]) - 1) + ':' + str(
                            max(e[0], d[0]) - 1);
                        if supInfo[2] == '+':  ## positive strand. alt-3 event
                            if key in ss3Events:  ## duplicate
                                ...
                                num3 += 1;
                                oFile_3.write(str(num3) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                    min(e[0], d[0]) - 1) + '\t' + str(e[1]) + '\t' + str(
                                    max(e[0], d[0]) - 1) + '\t' + str(e[1]) + '\t' + str(ce[0] - 1) + '\t' + str(
                                    ce[1]) + '\n');
                                if (tID.find("novel") >= 0):  # It involves novel junction
                                    neFile_3.write(str(num3) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                        min(e[0], d[0]) - 1) + '\t' + str(e[1]) + '\t' + str(
            ...
            for i in range(0, len(uf)):  ### going through the upstream flanking exons
                f1 = uf[i];  ## first flanking exon
                for tID in genes[gID]:  ## examine each transcript
                    if [f1[0], ce[1]] in genes[gID][
                        tID]:  ## there is an exon starts from f1 ends at ce, it is retained intron
                        ...
                        if key in riEvents:  ## already have this skipping events
                            ...
                            riEvents[key] = 1;
                            numRI += 1;
                            oFile_ri.write(str(numRI) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                f1[0] - 1) + '\t' + str(ce[1]) + '\t' + str(f1[0] - 1) + '\t' + str(f1[1]) + '\t' + str(
                                ce[0] - 1) + '\t' + str(ce[1]) + '\n');
                            if (tID.find("novel") >= 0):  # It involves novel junction
                                neFile_ri.write(str(numRI) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                    f1[0] - 1) + '\t' + str(ce[1]) + '\t' + str(f1[0] - 1) + '\t' + str(
                                    f1[1]) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\n');
            
            for i in range(0, len(df)):  ### going through the downstream flanking exons
                f1 = df[i];  ## first flanking exon
                for tID in genes[gID]:  ## examine each transcript
                    if [ce[0], f1[1]] in genes[gID][
                        tID]:  ## there is an exon starts from ce ends at f1, it is retained intron
                        ...
                        if key in riEvents:  ## already have this skipping events
                            ...
                            numRI += 1;
                            oFile_ri.write(str(numRI) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                ce[0] - 1) + '\t' + str(f1[1]) + '\t' + str(ce[0] - 1) + '\t' + str(ce[1]) + '\t' + str(
                                f1[0] - 1) + '\t' + str(f1[1]) + '\n');
                            if (tID.find("novel") >= 0):  # It involves novel junction
                                neFile_ri.write(str(numRI) + '\t' + gID + '\t' + '\t'.join(supInfo) + '\t' + str(
                                    ce[0] - 1) + '\t' + str(f1[1]) + '\t' + str(ce[0] - 1) + '\t' + str(
                                    ce[1]) + '\t' + str(f1[0] - 1) + '\t' + str(f1[1]) + '\n');
