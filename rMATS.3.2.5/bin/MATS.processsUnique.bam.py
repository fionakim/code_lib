...
s_afe = {};
s_ale = {};
s_ri = {};  ## supple dict. effective length of inclusion or skipping form for CT1,CT2,CT3
#
logline();
#
#### SE ######
...
for line in seFile:  ## process skipped exon events file
    ...
    strand = ele[4];
    tS = int(ele[5]);
    tE = int(ele[6]);  ## target exon coord
    uS = int(ele[7]);
    uE = int(ele[8]);  ## upstream exon coord
    dS = int(ele[9]);
    dE = int(ele[10]);  ## downstream exon coord
    ...
    I_2 = 0;
    S_2 = 0;  ## effective length considering paired-end info.. not implemented yet
    
    s_se[id] = [[I_0, S_0], [I_1, S_1], [I_2, S_2]];  ## effective length for CT1,CT2,CT3
    
    group = range(uS / chunk, uE / chunk + 1) + range(tS / chunk, tE / chunk + 1) + range(dS / chunk,
                                                                                          dE / chunk + 1);  ## groups this event could belong
...
for line in mxeFile:  ## process mxe events file
    ...
    tE = int(ele[6]);  ## target exon coord
    sS = int(ele[7]);
    sE = int(ele[8]);  ## second exon coord
    if strand == '-':  ## negative strand, switch target exon and second exon
        tS = int(ele[7]);
        tE = int(ele[8]);  ## target exon coord
        sS = int(ele[5]);
        sE = int(ele[6]);  ## second exon coord
    uS = int(ele[9]);
    uE = int(ele[10]);  ## upstream exon coord (samller coord)
    dS = int(ele[11]);
    dE = int(ele[12]);  ## downstream exon coord (bigger coord)
    ...
    group = group + range(sS / chunk, sE / chunk + 1) + range(dS / chunk,
                                                              dE / chunk + 1);  ## groups this event could belong
    group = list(set(group));  ## remove duplicate groups
...
logging.debug(
    "There are %d mxe ids in count dictionary and %d mxe ids in supple dictionary" % (len(c_mxe), len(s_mxe)));
#
logline();
#
#### A5SS ####
...
for line in a5ssFile:  ## process a5ss events file
    ...
    strand = ele[4];
    lS = int(ele[5]);
    lE = int(ele[6]);  ## long exon coord
    sS = int(ele[7]);
    sE = int(ele[8]);  ## short exon coord
    fS = int(ele[9]);
    fE = int(ele[10]);  ## flanking exon coord
...
logging.debug(
    "There are %d a5ss ids in count dictionary and %d a5ss ids in supple dictionary" % (len(c_a5ss), len(s_a5ss)));
#
logline();
#
#### A3SS ####
...
logging.debug(
    "There are %d a3ss ids in count dictionary and %d a3ss ids in supple dictionary" % (len(c_a3ss), len(s_a3ss)));
#
logline();
#
#### AFE ####
...
for line in riFile:  ## process ri events file
    ...
    strand = ele[4];
    rS = int(ele[5]);
    rE = int(ele[6]);  ## ri exon coord (including up- and down-stream exons)
    uS = int(ele[7]);
    uE = int(ele[8]);  ## upstream exon coord
    dS = int(ele[9]);
    dE = int(ele[10]);  ## downstream exon coord
        ...
        for read in sFile.fetch():
            ...
            if len(split_mString) == 2:
                ...
                if chr in mxe:  ## this chromosome has mxe event(s)
                    if group in mxe[chr]:  ## this group has mxe event(s)
                        for c in mxe[chr][group]:  ## for each mxe event in this group
                            if (mc > mxe[chr][group][c][0] and mec <= mxe[chr][group][c][
                                1]):  ## read on the target exon
                                c_mxe[c][CT2][sInd][I][rep] += 1;
                            elif (mc > mxe[chr][group][c][2] and mec <= mxe[chr][group][c][
                                3]):  ## read on the second exon
                ...
                if chr in a5ss:  ## this chromosome has a5ss event(s)
                    if egroup in a5ss[
                        chr]:  ## this egroup has a5ss event(s), this group is valid for positive strand only
                        for c in a5ss[chr][egroup]:  ## for each a5ss event in this group
                            
                            if a5ss[chr][egroup][c][4] > a5ss[chr][egroup][c][1]:  ## positive strand
                                if (mc <= (a5ss[chr][egroup][c][3] - (rL - junctionLength / 2) + 1) and mec <=
                                    a5ss[chr][egroup][c][1] and mec >= (a5ss[chr][egroup][c][3] + (
                                            rL - junctionLength / 2))):  ## multi-exon read supporting target
                                    c_a5ss[c][CT1][sInd][I][rep] += 1;
                                    c_a5ss[c][CT2][sInd][I][rep] += 1;
                                if (mc > a5ss[chr][egroup][c][3] and mec <= a5ss[chr][egroup][c][
                                    1]):  ## exon read supporting target
                                    c_a5ss[c][CT2][sInd][I][rep] += 1;
                    
                    if group in a5ss[
                        chr]:  ## this group has a5ss event(s), this group is valid for negative strand only
                        for c in a5ss[chr][group]:  ## for each a5ss event in this group
                            
                            if a5ss[chr][group][c][4] < a5ss[chr][group][c][1]:  ## negative strand
                                if (mc > a5ss[chr][group][c][0] and mc <= (
                                                a5ss[chr][group][c][2] - (rL - junctionLength / 2) + 1) and mec >= (
                                            a5ss[chr][group][c][2] + (
                                                    rL - junctionLength / 2))):  ## multi-exon read supporting target
                                    c_a5ss[c][CT1][sInd][I][rep] += 1;
                                    c_a5ss[c][CT2][sInd][I][rep] += 1;
                                if (mc > a5ss[chr][group][c][0] and mec <= a5ss[chr][group][c][
                                    2]):  ## exon read supporting target
                                    c_a5ss[c][CT2][sInd][I][rep] += 1;
                    ...
                    if egroup in a3ss[chr]:  ## this group has a3ss event(s)
                        for c in a3ss[chr][egroup]:  ## for each a3ss event in this group
                            ...
                            if a3ss[chr][egroup][c][4] > a3ss[chr][egroup][c][1]:  ## negative strand
                                if (mc <= (a3ss[chr][egroup][c][3] - (rL - junctionLength / 2) + 1) and mec <=
                                    a3ss[chr][egroup][c][1] and mec >= (a3ss[chr][egroup][c][3] + (
                                            rL - junctionLength / 2))):  ## multi-exon read supporting target
                                    c_a3ss[c][CT1][sInd][I][rep] += 1;
                                    c_a3ss[c][CT2][sInd][I][rep] += 1;
                                if (mc > a3ss[chr][egroup][c][3] and mec <= a3ss[chr][egroup][c][
                                    1]):  ## exon read supporting target
                                    c_a3ss[c][CT2][sInd][I][rep] += 1;
                    ...
                    if group in a3ss[chr]:  ## this group has a3ss event(s)
                        for c in a3ss[chr][group]:  ## for each a3ss event in this group
                            ...
                            if a3ss[chr][group][c][4] < a3ss[chr][group][c][1]:  ## positive strand
                                if (mc > a3ss[chr][group][c][0] and mc <= (
                                                a3ss[chr][group][c][2] - (rL - junctionLength / 2) + 1) and mec >= (
                                            a3ss[chr][group][c][2] + (
                                                    rL - junctionLength / 2))):  ## multi-exon read supporting target
                                    c_a3ss[c][CT1][sInd][I][rep] += 1;
                                    c_a3ss[c][CT2][sInd][I][rep] += 1;
                                if (mc > a3ss[chr][group][c][0] and mec <= a3ss[chr][group][c][
                                    2]):  ## exon read supporting target
                                    c_a3ss[c][CT2][sInd][I][rep] += 1;
                ...
                if chr in ri:  ## this chromosome has ri event(s)
                    ...
                    for ggg in groupsToExamine:  ## for each group
                        if ggg in ri[chr]:  ## this group has ri event(s)
                            for c in ri[chr][
                                ggg]:  ## for each ri event in this group, strand does not matter for ri events
                                if c in tempProcessedRI_id:  ## already processed this event, skip it
                                    continue;  ## next c please
                                else:  ## new c here
                                    tempProcessedRI_id[c] = '1';
                                    if (mc <= (ri[chr][ggg][c][3] - (rL - junctionLength / 2) + 1) and mec >= (
                                                ri[chr][ggg][c][3] + (rL - junctionLength / 2))) or (
                                                    mc <= (
                                                        ri[chr][ggg][c][4] - (rL - junctionLength / 2) + 1) and mec >= (
                                                        ri[chr][ggg][c][4] + (
                                                                rL - junctionLength / 2))):  ## multi-exon read supporting target
                                        c_ri[c][CT1][sInd][I][rep] += 1;
                                        c_ri[c][CT2][sInd][I][rep] += 1;
                                    if (mc > ri[chr][ggg][c][3] and mec <= ri[chr][ggg][c][
                                        4]):  ## exon read supporting target
                                        c_ri[c][CT2][sInd][I][rep] += 1;
                                        ## end of RI ###
                ...
                prevN = -1;
                nextN = -1;  ## for previous junction and next junction
                for ec in range(0, len(split_mString) - 2):  ## for each coordinate
                    ...
                    cStart = jS - secondNumber + 1;
                    cEnd = jS;
                    cStart_last = cStart;
                    cEnd_last = cEnd;
                    
                    minAnchor = min(int(split_mString[0]),
                                    int(split_mString[-2].split('N')[1]));  ## min nts going across junction
                    ...
                    if chr in se:  ## this chromosome has se event(s)
                        if group in se[chr]:  ## this group has skipped exon event(s)
                            for c in se[chr][
                                group]:  ## for each skipped exon event in this group, examine if the given junction is part of it
                                ...
                                if (jS == se[chr][group][c][3] and jE == se[chr][group][c][0] and (
                                            (lastNumber <= seLength and nextN == -1) or (
                                                        lastNumber == seLength and nextN == dnjLength))) or (
                                                    jS == se[chr][group][c][1] and jE == se[chr][group][c][4] and (
                                                    (secondNumber <= seLength and prevN == -1) or (
                                                                secondNumber == seLength and prevN == upjLength))):  ## IJC
                                    key = ':'.join(["SE", str(c)]);
                    ...
                    if chr in mxe:  ## this chromosome has mxe event(s)
                        if group in mxe[chr]:  ## this group has mxe event(s)
                            for c in mxe[chr][
                                group]:  ## for each mxe event in this group, examine if the given junction is part of it
                                ...
                                if (jS == mxe[chr][group][c][5] and jE == mxe[chr][group][c][0] and (
                                            (lastNumber <= mxeLen1 and nextN == -1) or (
                                                        lastNumber == mxeLen1 and nextN == dnj1Len))) or (
                                                    jS == mxe[chr][group][c][1] and jE == mxe[chr][group][c][6] and (
                                                    (secondNumber <= mxeLen1 and prevN == -1) or (
                                                                secondNumber == mxeLen1 and prevN == upj1Len))):  ## IJC
                                    key = ':'.join(["MXE", str(c)]);
                                    if key in usedE:  ## already used;
                                        continue;  ## skip and go to next c
                                    ...
                                    c_mxe[c][CT2][sInd][I][rep] += 1;
                                    usedE[key] = 1;  ## okay to overwrite
                                elif (jS == mxe[chr][group][c][5] and jE == mxe[chr][group][c][2] and (
                                            (lastNumber <= mxeLen2 and nextN == -1) or (
                                                        lastNumber == mxeLen2 and nextN == dnj2Len))) or (
                                                    jS == mxe[chr][group][c][3] and jE == mxe[chr][group][c][6] and (
                                                    (secondNumber <= mxeLen2 and prevN == -1) or (
                                                                secondNumber == mxeLen2 and prevN == upj2Len))):  ## SJC
                                    key = ':'.join(["MXE", str(c)]);
                                    if key in usedE:  ## already used;
                                        continue;  ## skip and go to next c
                    ...
                    if chr in a5ss:  ## this chromosome has a5ss event(s)
                        if group in a5ss[chr]:  ## this group has a5ss event(s)
                            for c in a5ss[chr][
                                group]:  ## for each a5ss event in this group, examine if the given junction is part of it
                                if a5ss[chr][group][c][4] > a5ss[chr][group][c][1]:  ## positive strand
                                    if (jS == a5ss[chr][group][c][1] and jE == a5ss[chr][group][c][4]):  ## IJC
                                        ...
                                        cStart_last = jE + 1;
                                        cEnd_last = jE + lastNumber;
                                        if (cStart_last <= (
                                                        a5ss[chr][group][c][3] - (
                                                        rL - junctionLength / 2) + 1) and cEnd_last <=
                                            a5ss[chr][group][c][1] and cEnd_last >= (a5ss[chr][group][c][3] + (
                                                    rL - junctionLength / 2))):  ## multi-exon read supporting target
                                            key = ':'.join(["A5SS", str(c)]);
                                    ...
                                    if (jS == a5ss[chr][group][c][5] and jE == a5ss[chr][group][c][0]):  ## IJC
                                        ...
                                        if (cStart > a5ss[chr][group][c][0] and cStart <= (
                                                        a5ss[chr][group][c][2] - (
                                                        rL - junctionLength / 2) + 1) and cEnd >= (
                                                    a5ss[chr][group][c][2] + (
                                                            rL - junctionLength / 2))):  ## multi-exon read supporting target
                                            key = ':'.join(["A5SS", str(c)]);
                                            if key not in usedE:  ## read didn't cover this event before, add it
                    ...
                    if chr in a3ss:  ## this chromosome has a3ss event(s)
                        if group in a3ss[chr]:  ## this group has a3ss event(s)
                            for c in a3ss[chr][
                                group]:  ## for each a3ss event in this group, examine if the given junction is part of it
                                if a3ss[chr][group][c][4] > a3ss[chr][group][c][1]:  ## negative strand
                                    if (jS == a3ss[chr][group][c][1] and jE == a3ss[chr][group][c][4]):  ## IJC
                                        ...
                                        if (cStart_last <= (
                                                        a3ss[chr][group][c][3] - (
                                                        rL - junctionLength / 2) + 1) and cEnd_last <=
                                            a3ss[chr][group][c][1] and cEnd_last >= (a3ss[chr][group][c][3] + (
                                                    rL - junctionLength / 2))):  ## multi-exon read supporting target
                                            key = ':'.join(["A3SS", str(c)]);
                                            if key not in usedE:  ## read didn't cover this event before, add it
                                                c_a3ss[c][CT1][sInd][I][rep] += 1;
                                    ...
                                    if (jS == a3ss[chr][group][c][5] and jE == a3ss[chr][group][c][0]):  ## IJC
                                        ...
                                        if (cStart > a3ss[chr][group][c][0] and cStart <= (
                                                        a3ss[chr][group][c][2] - (
                                                        rL - junctionLength / 2) + 1) and cEnd >= (
                                                    a3ss[chr][group][c][2] + (
                                                            rL - junctionLength / 2))):  ## multi-exon read supporting target
                    ...
                    if chr in ri:  ## this chromosome has ri event(s)
                        if group in ri[chr]:  ## this group has ri event(s)
                            for c in ri[chr][
                                group]:  ## for each ri event in this group, examine if the given junction is part of it
                                if jS == ri[chr][group][c][3] and jE == ri[chr][group][c][4]:  ## SJC
                                    ...
                                    usedE[key] = 1;  ## okay to overwrite
                                if prevN == -1:
                                    if cStart <= (ri[chr][group][c][4] - (rL - junctionLength / 2) + 1) and cEnd >= (
                                                ri[chr][group][c][4] + (
                                                        rL - junctionLength / 2)):  ## multi-exon read supporting target
                                        key = ':'.join(["RI", str(c)]);
                                        if key not in usedE:  ## read didn't cover this event before, add it
                                ...
                                if nextN == -1:
                                    cStart_last = jE + 1;
                                    cEnd_last = jE + lastNumber;  ## just change cStart and cEnd then do the same process
                                    if cStart_last <= (
                                                    ri[chr][group][c][3] - (
                                                    rL - junctionLength / 2) + 1) and cEnd_last >= (
                                                ri[chr][group][c][3] + (rL - junctionLength / 2)):
                                        key = ':'.join(["RI", str(c)]);
                                        if key not in usedE:  ## read didn't cover this event before, add it
                                            c_ri[c][CT1][sInd][I][rep] += 1;
                                            c_ri[c][CT2][sInd][I][rep] += 1;
...
def processSample_stranded(sample, sInd, dt,
                           lt):  ## call it with processSample_firststrand(sample_1, S1,dataType,libraryType) something like this
    global anchorLength;
                                ...
                                if a5ss[chr][group][c][4] > a5ss[chr][group][c][1]:  ## positive strand
                                    if (jS == a5ss[chr][group][c][1] and jE == a5ss[chr][group][c][4]):  ## IJC
                                        ...
                                        if (cStart <= (
                                                        a5ss[chr][group][c][3] - (
                                                        rL - junctionLength / 2) + 1) and cEnd <=
                                            a5ss[chr][group][c][1] and cEnd >= (a5ss[chr][group][c][3] + (
                                                    rL - junctionLength / 2))):  ## multi-exon read supporting target
                                            key = ':'.join(["A5SS", str(c)]);
                                            if key not in usedE:  ## read didn't cover this event before, add it
                                                c_a5ss[c][CT1][sInd][I][rep] += 1;
                                ...
                                if a3ss[chr][group][c][4] > a3ss[chr][group][c][1]:  ## negative strand
                                    if (jS == a3ss[chr][group][c][1] and jE == a3ss[chr][group][c][4]):  ## IJC
                                        ...
                                        if (cStart <= (
                                                        a3ss[chr][group][c][3] - (
                                                        rL - junctionLength / 2) + 1) and cEnd <=
                                            a3ss[chr][group][c][1] and cEnd >= (a3ss[chr][group][c][3] + (
                                                    rL - junctionLength / 2))):  ## multi-exon read supporting target
                                            key = ':'.join(["A5SS", str(c)]);
                                            if key not in usedE:  ## read didn't cover this event before, add it
                                                c_a3ss[c][CT1][sInd][I][rep] += 1;
                    ...
                    if chr in ri:  ## this chromosome has ri event(s)
                        if group in ri[chr]:  ## this group has ri event(s)
                            for c in ri[chr][
                                ...
                                if nextN == -1:
                                    cStart = jE + 1;
                                    cEnd = jE + lastNumber;  ## just change cStart and cEnd then do the same process
                                    if cStart <= (ri[chr][group][c][3] - (rL - junctionLength / 2) + 1) and cEnd >= (
                                                ri[chr][group][c][3] + (rL - junctionLength / 2)):
                                        key = ':'.join(["RI", str(c)]);
                                        if key not in usedE:  ## read didn't cover this event before, add it
                                            c_ri[c][CT1][sInd][I][rep] += 1;
...
def writeInputFile(h1, h2, h3, f1, f2, cnt, sup):  ## header 1,2,3, file 1,2,3, count dict, supple dict)
    ...
    for k in sorted(sup.keys()):
        f1.write(str(k) + '\t' + ','.join(map(str, cnt[k][CT1][S1][I])) + '\t' + ','.join(
            map(str, cnt[k][CT1][S1][S])) + '\t' + ','.join(map(str, cnt[k][CT1][S2][I])) + '\t' + ','.join(
            map(str, cnt[k][CT1][S2][S])) + '\t' + str(sup[k][CT1][0]) + '\t' + str(sup[k][CT1][1]) + '\n');
        f2.write(str(k) + '\t' + ','.join(map(str, cnt[k][CT2][S1][I])) + '\t' + ','.join(
            map(str, cnt[k][CT2][S1][S])) + '\t' + ','.join(map(str, cnt[k][CT2][S2][I])) + '\t' + ','.join(
            map(str, cnt[k][CT2][S2][S])) + '\t' + str(sup[k][CT2][0]) + '\t' + str(sup[k][CT2][1]) + '\n');
