                ...
                if ref_txpt_id.strip() in txpt_gname_gid_dic.keys():
                    ...
                    if re.match(r'^[^u]$', combined_cls.strip()):
                        newline = re.sub(r'(\s*)(gene_id)(\s+)\"(\S+)\";',
                                         '\g<1>\g<2>\g<3>\"{},{}\"'.format(ref_gid, ref_gname), newline)
    
                    if re.match(r'^[^=u]$', combined_cls.strip()):
                        newline = re.sub(r'(\s*)(transcript_id)(\s+)\"(\S+)\";',
                                         '\g<1>\g<2>\g<3>\"{},{}\"'.format(internal_tid, ref_gname),
                                         newline)
                    ...
                    raise Exception('ref gtf file has no record for txpt id: {}'.format(ref_txpt_id))
            else:
                newline = line
                fw.write(newline.strip(';\s')+';\n')
