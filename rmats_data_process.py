...
PRIMARY_DIRS = {'ASEvents', 'commands.txt', 'config.txt', 'MATS_output', 'SAMPLE_1', 'SAMPLE_2', 'summary.txt'}
...
def get_event_stats(files):
    ...
    for k in LEGAL_EVENT_TYPE:
        d[k] = {'all_event_file': None, 'novel_event_file': None, 'JunctionCountOnly_file': None,
                ...
                'all_event_id_no': 0, 'novel_event_id_no': 0, 'old_event_id_no': 0,
...
def write_psi_detail_file(data_src, general_psi_dic, detail_file):
    detail_fw = open(detail_file, 'wb')
    detail_fw.write('id\tevent_type\tPValue\tFDR\tinclevels_SAMPLE1\tinclevels_SAMPLE2\t'
                    'aver_inclevel_SAMPLE1\taver_inclevel_SAMPLE2\tdelta_psi_s1-s2\tincrease_inclusion(case:SAMPLE1)\t'
                    'increase_exclusion(case:SAMPLE1)\tincrease_inclusion(case:SAMPLE2)\tincrease_exclusion(case:SAMPLE2)\n')
    for event in LEGAL_EVENT_TYPE:
        info = general_psi_dic[event][data_src + "_data"]
        for e_id in info.keys():
            (inclusion_s1, exclusion_s1, inclusion_s2, exclusion_s2) = ('', '', '', '')
            if isnumber(info[e_id][6]):
                if float(info[e_id][6]) > 0:
                    (inclusion_s1, exclusion_s1, inclusion_s2, exclusion_s2) = ('是', '否', '否', '是')
                else:
                    (inclusion_s1, exclusion_s1, inclusion_s2, exclusion_s2) = ('否', '是', '是', '否')
            else:
                print('不是浮点数的delta psi 值: 事件{} '.format(e_id))
            newline = '{}\n'.format('\t'.join([str(e) for e in
                                               [e_id, event, info[e_id][0], info[e_id][1], info[e_id][2], info[e_id][3],
                                                info[e_id][4], info[e_id][5],
                                                info[e_id][6], inclusion_s1, exclusion_s1, inclusion_s2,
                                                exclusion_s2]]))
            detail_fw.write(newline)
    detail_fw.close()


def write_psi_stats_file(stat_file, general_psi_dic):
    stat_fw = open(stat_file, 'wb')
    stat_fw.write(
        '#stats: case=SAMPLE1\tincrease_inclusion_events_count_from_JunctionCountOnly_data=%d\tincrease_exclusion_events_count_from_JunctionCountOnly_data=%d\n' % (
            general_psi_dic['SAMPLE_1']['JunctionCountOnly']['inclusion_total'],
            general_psi_dic['SAMPLE_1']['JunctionCountOnly']['inclusion_total']))
    stat_fw.close()
...
def new_write_psi_detail_file(file_src_dic):
    for as_type in LEGAL_EVENT_TYPE:
...
if __name__ == '__main__':
    out_dir = '/mnt/hgfs/I/rmats-out-eg/20170420'
    case_name = 'SAMPLE_1'
    control_name = 'SAMPLE_2'
    detail_jc_psi = os.path.join(out_dir, 'JunctionCountOnly_detail_table.txt')
    detail_all_reads_psi = os.path.join(out_dir, 'ReadsOnTargetAndJunctionCounts_detail_table.txt')
    stat_psi = os.path.join(out_dir, 'psi_stats_table.txt')
    ...
    psi_info_dic = get_event_psi_dic(d=event_info_dic)

    # write_event_stat_file(event_stat_f=events_stats_file, event_stat_dic=event_info_dic)
    new_write_psi_detail_file(file_src_dic=event_info_dic)
    # write_psi_stats_file(stat_file=stat_psi, general_psi_dic=psi_info_dic)
